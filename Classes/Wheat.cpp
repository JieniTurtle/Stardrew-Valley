...
bool Wheat::init(TMXTiledMap* map) {
    ...
    setwheatcheckbox();//设置锄头复选框
    showwheatnum();//显示种子数量
    //wheatListenerMouse(map);
    return true;
}

void  Wheat::setwheatcheckbox() {
    auto visibleSize = Director::getInstance()->getVisibleSize();//获取当前游戏视图窗口的尺寸
    //创建锄头图标
    wheatcheckbox = ui::CheckBox::create("wheat1.png", "wheat2.png");
    wheatcheckbox->setPosition(Vec2(visibleSize.width * 9/ 10, visibleSize.height*7 /10)); // 设置位置
    ...
    //    }
    //    });
    this->addChild(wheatcheckbox);


}

Wheat* Wheat::create(TMXTiledMap* map) {
