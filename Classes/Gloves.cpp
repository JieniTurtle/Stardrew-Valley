...
bool Gloves::init(TMXTiledMap* map, int& wheatnum) {
    isgloves = 0;
    ...
    glovesListenerMouse(map,wheatnum);
    return true;
}
...
void  Gloves::setglovescheckbox() {
    ...
    glovescheckbox->setPosition(Vec2(visibleSize.width *2/ 8, visibleSize.height / 4)); // 设置位置
...
Gloves* Gloves::create(TMXTiledMap* map, int& wheatnum) {

    Gloves* ret = new Gloves();
    if (ret && ret->init(map,wheatnum)) {
...
void Gloves::glovesListenerMouse(TMXTiledMap* map, int& wheatnum) {
    // 创建鼠标事件监听器
    auto mouseListener = EventListenerMouse::create();

    mouseListener->onMouseDown = [=, & wheatnum](Event* event) {
