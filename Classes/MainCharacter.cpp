...
void MainCharacter::update(float delta) {
    ...
    if (movementkeys[UP] + movementkeys[DOWN] + movementkeys[RIGHT] + movementkeys[LEFT] == 0) {
        frame_count = 4 * 60 / frame_per_second - 1;
    }
    ...
    if (this->getPosition().x <= 0)
        direction = LEFT;
    else if (this->getPosition().x >= mainmap->getMapSize().width * mainmap->getTileSize().width)
        direction = RIGHT;
    else if (this->getPosition().y >= mainmap->getMapSize().height * mainmap->getTileSize().height)
        direction = UP;
